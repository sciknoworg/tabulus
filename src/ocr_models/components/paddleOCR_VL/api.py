...
def _extract_refs(text: str):
    ...
    # small cleanup for broken DOI lines
    text = re.sub(
        r'(?i)(doi\s*:\s*10\.[^\n]+)\n\s*([0-9A-Za-z.\-_/]+)',
    ...
    def is_safe_continuation(line: str) -> bool:
        ...
        # lowercase / punctuation / bracket start usually continuation
        if re.match(r"^[a-zà-öø-ÿ\(\[\{]", s):
            return True

        # journal tail / volume / pages style
        ...
        # line that obviously starts mid-sentence
    ...
    def looks_like_new_reference(line: str) -> bool:
        ...
        # numbered style: "93. Edgerton..."
        if numbered_start.match(s) and year_pattern.search(s):
            return True

        # most bibliography lines start uppercase and contain a year
    ...
    def finalize():
        ...
        # remove trailing garbage if it got appended
        ref = re.split(
    ...
    for line in lines:
        ...
        # if line is clearly continuation -> append
        if is_safe_continuation(line):
            current.append(line)
            continue

        # if line looks like a fresh ref -> finalize previous
        if looks_like_new_reference(line):
            finalize()
            current = [line]
            continue
...
print("[api] loading PaddleOCRVL...", flush=True)
# Official API shown in PaddleOCR-VL docs/model card uses PaddleOCRVL class. :contentReference[oaicite:3]{index=3}
pipeline = PaddleOCRVL()
print("[api] PaddleOCRVL loaded ✅", flush=True)
...
def _get_markdown_from_res(res) -> str:
    """
    Try common places PaddleOCR pipelines store markdown.
    If not available, fallback to string.
    """
    md = None
    if hasattr(res, "markdown"):
        md = res.markdown
...
def _extract_markdown_tables(text: str) -> list[str]:
    ...
    consecutive lines with pipes, including a separator row with ---.
    """
...
def _markdown_table_to_rows(md_table: str) -> list[list[str]]:
    """
    Parse a markdown table into rows (including header as first row).
...
async def ocr_pdf(file: UploadFile = File(...)):
    ...
    for i in range(n_pages):
        ...
        # PaddleOCRVL predicts on an image path / image. :contentReference[oaicite:4]{index=4}
        output = pipeline.predict(str(img_path))

        print(f"[api] page {page_nr}/{n_pages}: predict DONE ({time.time()-t_page:.1f}s)", flush=True)

        # output may be iterable; take first result if needed
        res0 = None
        try:
        ...
        # tables: best-effort from HTML inside markdown
...
async def ocr_images(files: List[UploadFile] = File(...)):
    """
    Input: multiple PNG/JPG images (table crops)
    Output: tables in JSON rows
    """
    t_start = time.time()
    ...
    for idx, uf in enumerate(files, start=1):
        ...
        # Prefer HTML tables in markdown
        html_tables = _extract_html_tables(md_text)
