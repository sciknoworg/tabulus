...
in the Stage 4 bibliography artifact.
...
The matcher writes table-cell links. Stage 6 aggregates
`matched_reference_indices` across supplied Stage 5 artifacts at paper scope,
...
See {doc}`../tutorial/13-doi-resolution` for Stage 6 and
{doc}`../data-contracts/reference-matches-json` for the Stage 5 artifact.
