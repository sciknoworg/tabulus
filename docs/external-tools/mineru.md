...
MinerU `table_body` is retained as a native reconstruction candidate produced during profiling. The crop-consuming reconstruction adapters, such as PaddleOCR-VL, Chandra OCR 2, NuExtract3, Tesseract + Table Transformer, RapidOCR + Docling TableFormer, and Granite Vision 4.1 4B, operate later on the canonical crop images rather than on the original PDF.
