...
The rebuilt library now provides standalone CLI commands for profiling, table-crop export, and batch table reconstruction. It does not yet provide a complete end-to-end `tabulus run` command. This page describes the target workflow shape.
...
tabulus export-table-crops \
  ...
  --out /data/papers/tabulus-output/table-crops/P51
...
```bash
tabulus run --pdf /data/papers/P51.pdf --runs-root /data/runs
```
