...
| --- | --- | --- |
| `png` | `content_list.json` table entry `img_path` | Tabulus copies the referenced MinerU-generated image into the run output. |
| `mineru_img_path` | `content_list.json` table entry `img_path` | Preserve the original adapter path for traceability. |
| `page_nr` | `content_list.json` table entry `page_idx` | MinerU uses a zero-based page index. The normalized contract should document whether it preserves zero-based numbering or converts to one-based page numbers. |
