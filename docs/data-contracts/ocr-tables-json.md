...
`native_json` and `native_markdown` preserve adapter-native result
representations for provenance and debugging. Depending on the adapter, this
may include model metadata, prompt and generation settings, OCR tokens,
bounding boxes retained as provenance, OTSL, HTML, Markdown, JSON layout
objects, table-structure objects, dependency versions, and parser-input
policy. These native fields are evidence, not independent prediction CSVs.
For adapter-specific native evidence, see the relevant External Tools page.
