...
The implemented library is designed so most unit tests do not require GPU
access. Tests cover MinerU output discovery, profiling command construction,
automatic table-crop export, the table reconstruction adapter registry,
mocked adapter behavior, shared HTML/Markdown/OTSL parsing, batch
reconstruction, artifact writing, and reference-table classification.
...
- registered reconstruction adapter behavior with mocked dependencies
...
The table reconstruction adapter package is available as `tabulus.table_ocr`.
Its ML dependencies are optional and loaded only when the selected adapter is
instantiated. For the current adapter list, see
{doc}`../tutorial/08-table-ocr`.

`tabulus reconstruct-tables` runs one registered table-reconstruction adapter
across every crop in a canonical `tables_index.json` handoff:
